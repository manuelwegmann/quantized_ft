"""
LoRA fine-tuning diagnostic.

Same setup as run_supervised_ft.py but updates the backbone via low-rank
adapters (LoRA) instead of full weight updates. All original backbone weights
are frozen; only the rank-decomposition matrices A and B are trained.

This isolates whether parameter efficiency matters for domain adaptation,
and establishes whether LoRA can match full fine-tuning with far fewer
trainable parameters — relevant for the low-resource adaptation goal.

Usage:
    python scripts/run_lora_ft.py
    python scripts/run_lora_ft.py --n_train 300 --epochs 5 --lora_rank 8
"""
...
class LoRALinear(nn.Module):
    """Drop-in replacement for nn.Linear with a frozen base and trainable low-rank update."""
...
def apply_lora(backbone: nn.Module, rank: int, alpha: float) -> int:
    """
    Replace every nn.Linear in backbone with a LoRALinear.
...
def main():
    ...
    parser.add_argument("--lr",         type=float, default=1e-3,
                        help="LR for LoRA params and head (base weights are frozen).")
    ...
    print("=== LoRA fine-tuning diagnostic ===")
    print(f"  n_train={args.n_train}  epochs={args.epochs}  "
          f"rank={args.lora_rank}  alpha={args.lora_alpha}  lr={args.lr:.1e}")
    ...
    tr_indices = [labeled[i] for i in perm[:n_tr_full]][:args.n_train]
